...
public class EntityRenderer implements IResourceManagerReloadListener
{
    ...
    public void getMouseOver(float partialTicks)
    {
        ...
        {
            Reach reach = (Reach) Fraction.INSTANCE.getModuleManager().getModule(Reach.class);

            this.mc.mcProfiler.startSection("pick");
            this.mc.pointedEntity = null;
            double blockReach = Math.max(this.mc.playerController.getBlockReachDistance(), reach.getReach());
            this.mc.objectMouseOver = entity.rayTrace(blockReach, partialTicks);
            double hitDistance = blockReach;
            ...
            {
                hitDistance = this.mc.objectMouseOver.hitVec.distanceTo(eyePosition);
            }

            Vec3 lookVector = entity.getLook(partialTicks);
            Vec3 vec32 = eyePosition.addVector(lookVector.xCoord * blockReach, lookVector.yCoord * blockReach, lookVector.zCoord * blockReach);
            this.pointedEntity = null;
            Vec3 vec33 = null;
            float f = 1.0F;
            List<Entity> list = this.mc.theWorld.getEntitiesInAABBexcluding(entity, entity.getEntityBoundingBox().addCoord(lookVector.xCoord * blockReach, lookVector.yCoord * blockReach, lookVector.zCoord * blockReach).expand((double)f, (double)f, (double)f), Predicates.and(EntitySelectors.NOT_SPECTATING, new Predicate<Entity>()
            {
            ...
            double hitDistance2 = hitDistance;

            HitBox hitBox = (HitBox) Fraction.INSTANCE.getModuleManager().getModule(HitBox.class);

            for (Entity value : list) {
                float f1 = value.getCollisionBorderSize() + hitBox.getExpand();
                AxisAlignedBB axisalignedbb = value.getEntityBoundingBox().expand(f1, f1, f1);
                MovingObjectPosition movingobjectposition = axisalignedbb.calculateIntercept(eyePosition, vec32);

                if (axisalignedbb.isVecInside(eyePosition)) {
                    if (hitDistance2 >= 0.0D) {
                        this.pointedEntity = value;
                        vec33 = movingobjectposition == null ? eyePosition : movingobjectposition.hitVec;
                        hitDistance2 = 0.0D;
                    }
                } else if (movingobjectposition != null) {
                    double d3 = eyePosition.distanceTo(movingobjectposition.hitVec);

                    if (d3 < hitDistance2 || hitDistance2 == 0.0D) {
                        boolean flag1 = false;
            ...
            if (this.pointedEntity != null && flag && eyePosition.distanceTo(vec33) > reach.getReach())
            {
                this.pointedEntity = null;
            ...
            if (this.pointedEntity != null && (hitDistance2 < hitDistance || this.mc.objectMouseOver == null))
            {
